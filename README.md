...
仓库当前候选版本为 **backend r32 / LuCI r13**。r32 首次采用完整访问默认值：
U30 Pro 已实现的管理操作、设备原生智能充电和对应 UCI 门控均默认启用；从旧版
只读配置升级时会执行一次性 `full_v1` 迁移，迁移完成后仍尊重用户后续手动关闭的
功能。当前源码已通过本地全量模拟与回归；双 SDK、QEMU 和新一轮真机验证仍按顺序
执行，不能沿用历史 r30 的只读证据。离线结果见
[U30 离线代码门禁](docs/validation/2026-08-04-u30-offline-code-gate.md)：

- 已接入 U25S goform 双重 SHA-256 登录和批量状态读取。
- 已接入 U30 Pro 的 CDC-NCM 自动识别、HTTPS 状态读取、设备原生
  `power_supply_mode` 状态，以及默认启用的手动/自动智能充电执行链。自动策略具有
  低/高阈值滞回、严格写门控、单次 POST、强制安全读回和失败冷却。
- 已按目标 U30 Pro WebUI 源码实现 APN、连接模式、2.4 GHz Wi-Fi、流量套餐、
  流量清零、短信发送/删除/已读、重启和关机的请求与读回链；这些已实现 capability
  在 U30 Pro 配置档中默认开放。目标配置明确不支持 5 GHz，
  因此不会从通用固件代码臆造该能力。
...
- 发布配置默认 `write_enabled=1`，U30 Pro 已实现的独立动作门控及智能充电均为 1。
- U25S SIM 切换已开放；没有对应实现或设备不支持的能力不会因全局开关而被伪造。
...
- 阶段 2 的九类语义写请求契约已补齐：SIM、APN、连接模式、Wi-Fi、流量计划、
  ...
  事件日志中。U30 Pro 对应 capability 已按本地状态化模拟结果开放；U25S 没有实现
  的同名操作继续按设备配置档保持不可用。实施记录见
  [写请求契约计划](docs/superpowers/plans/2026-08-01-write-request-contracts.md)。
- LuCI 已为上述九类请求补齐 capability 门控表单、危险操作确认、重复提交保护
  和统一异步结果跟踪；r32 发布配置会直接显示 U30 Pro 已实现的操作。
...
- r30 之后的当前源码已通过状态化 U30 模拟器，覆盖供电模式、APN、连接模式、
  Wi-Fi、流量套餐/清零、短信发送/删除/已读、重启和关机的真实生产调用链；
  明确区分设备拒绝、发送前失败和结果不确定，非幂等 POST 不自动重试。该结论
  仅属于离线代码门禁，不代表 SDK 包或真机能力已经验收。
...
| OpenWrt | 包格式 | 最近完成构建验收的 r30 / LuCI r12 | 验证结果 |
|---|---|---|---|
...
历史 backend r30 / LuCI r12 已完成本地检查、官方双版本 SDK 构建、隔离 QEMU
...
当前分支的新增源码不是上述 r30 二进制包；最终 SDK/QEMU/真机门禁通过后才会统一
升级包版本、生成新产物并更新此表。
...
当前源码对应的下一预发布标签为 `v0.1.0-rc1-r32`，尚未发布；只有维护者显式创建并推送与包元数据
