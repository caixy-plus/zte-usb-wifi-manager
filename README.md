...
仓库目前处于**U25S / U30 Pro 设备控制台整合开发预览阶段**。包元数据仍为
backend r31 / LuCI r12；历史 r30 已完成官方双 SDK、双版本隔离 QEMU 和路由器
只读验证。r31 完成了写入崩溃恢复、智能充电事务恢复和 U30 全动作状态化 HTTP
模拟门禁，但尚未构建正式包。新源码的 SDK/QEMU/真机验证
统一留到最终调试阶段，不能沿用 r30 证据。离线结果见
...
当前源码对应的下一预发布标签为 `v0.1.0-rc1-r31`，尚未发布；只有维护者显式创建并推送与包元数据
