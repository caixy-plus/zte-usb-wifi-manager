...
仓库目前处于**U25S / U30 Pro 设备控制台整合开发预览阶段**。源码 backend r25 / LuCI r12
已完成本地离线自动化验证、OpenWrt 25.12.5 / 24.10.7 官方双 SDK 构建，以及两套
隔离 QEMU 中从 backend r15 / LuCI r4 原位升级的完整生命周期验证。最近完成路由器
验证的发布包仍是 backend r15 / LuCI r4；r25/r12 尚未进行最后的真机调试验收：
...
当前源码对应的下一预发布标签为 `v0.1.0-rc1-r25`，尚未发布；只有维护者显式创建并推送与包元数据
