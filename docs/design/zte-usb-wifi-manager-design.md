...
本产品是一个**通过 USB 接入 OpenWrt 的中兴随身 WiFi 管理工具**。它将原本分散在中兴设备后台、OpenWrt 网络状态和板级 USB 控制中的信息与操作，集中到一个 LuCI 功能入口中。

电池与供电管理是特色模块，但不是产品的全部定位。产品还负责移动网络、SIM、流量、短信、U25S 自身 Wi-Fi、客户端、设备动作、诊断和故障保护。
...
4. 提供 USB 供电控制、电池阈值维护、离岗前充电和手动立即充满。
5. 与现有 `zte-usb-recover` 服务协调，避免计划断电被当作 USB 故障。
6. 当设备 API、守护进程或控制后端异常时，优先恢复 USB 供电，避免网络因电池耗尽长期中断。
...
2. 对用户密码执行 SHA-256，得到十六进制摘要。
3. 将第一步摘要与 `LD` 拼接，再执行一次 SHA-256。
4. 将最终摘要转为大写。
5. 提交登录：
...
- 完成板级 Power Adapter 探测。
- 完成状态机、日程、立即充满和故障保护。
- 完成 `zte-usb-recover` inhibit 协调。
