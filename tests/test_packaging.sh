...
assert_eq '0.1.0_rc1|21|10|v0.1.0-rc1-r21|prerelease' \
...
case " $* " in
    ...
    *' package/zte-usb-wifi-manager/compile '*)
        ...
        printf 'apk-backend\n' \
            >bin/packages/fixture/zte-usb-wifi-manager-0.1.0_rc1-r21.apk
        printf 'ipk-backend\n' \
            >bin/packages/fixture/zte-usb-wifi-manager_0.1.0_rc1-r21_all.ipk
        ;;
    *' package/luci-app-zte-usb-wifi-manager/compile '*)
        printf 'apk-luci\n' \
            >bin/packages/fixture/luci-app-zte-usb-wifi-manager-0.1.0_rc1-r10.apk
        printf 'ipk-luci\n' \
            >bin/packages/fixture/luci-app-zte-usb-wifi-manager_0.1.0_rc1-r10_all.ipk
        ;;
esac
EOF
chmod +x "$fake_bin"/*
...
printf apk-backend >"$work/incoming/packages-25.12.5/zte-usb-wifi-manager-0.1.0_rc1-r21.apk"
printf apk-luci >"$work/incoming/packages-25.12.5/luci-app-zte-usb-wifi-manager-0.1.0_rc1-r10.apk"
printf ipk-backend >"$work/incoming/packages-24.10.7/zte-usb-wifi-manager_0.1.0_rc1-r21_all.ipk"
printf ipk-luci >"$work/incoming/packages-24.10.7/luci-app-zte-usb-wifi-manager_0.1.0_rc1-r10_all.ipk"
...
assert_success node scripts/assemble-openwrt-packages.js \
    "$work/incoming" "$work/dist-r21-tag" "$source_sha" v0.1.0-rc1-r21
assert_success node -e '
const fs = require("fs");
const manifest = JSON.parse(fs.readFileSync(process.argv[1]));
if (manifest.project_ref !== "v0.1.0-rc1-r21" ||
    manifest.project_tag !== "v0.1.0-rc1-r21")
    process.exit(1);
' "$work/dist-r21-tag/build-manifest.json"
...
mv "$work/wrong-version-incoming/packages-25.12.5/zte-usb-wifi-manager-0.1.0_rc1-r21.apk" \
...
if grep -Fqx "      - 'v0.1.0-rc1'" "$workflow"; then
    fail 'r21 workflow must not publish the historical release tag'
else
    pass
fi
if grep -Fiq 'read-only developer preview' "$workflow"; then
    fail 'r21 release notes must not claim the package is read-only'
else
    pass
fi
...
if grep -Eq 'Backend r[89]([^0-9]|$)' "$workflow"; then
    fail 'r21 workflow must not retain stale r8 or r10 release notes'
else
    pass
fi
