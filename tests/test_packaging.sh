...
case " $* " in
    ...
    *' package/zte-usb-wifi-manager/compile '*)
        ...
        printf 'apk-backend\n' \
            >bin/packages/fixture/zte-usb-wifi-manager-0.1.0_rc1-r40.apk
        printf 'ipk-backend\n' \
            >bin/packages/fixture/zte-usb-wifi-manager_0.1.0_rc1-r40_all.ipk
        ;;
...
assert_success node scripts/assemble-openwrt-packages.js \
    "$work/incoming" "$work/dist-r24-tag" "$source_sha" v0.1.0-rc1-r40
assert_success node -e '
const fs = require("fs");
const manifest = JSON.parse(fs.readFileSync(process.argv[1]));
if (manifest.project_ref !== "v0.1.0-rc1-r40" ||
    manifest.project_tag !== "v0.1.0-rc1-r40")
    process.exit(1);
...
mv "$work/wrong-version-incoming/packages-25.12.5/zte-usb-wifi-manager-0.1.0_rc1-r40.apk" \
