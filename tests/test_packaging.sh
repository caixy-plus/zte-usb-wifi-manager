...
assert_file_contains "$builder" \
    "zte-usb-wifi-manager-0\\.1\\.0_rc1-r13\\.apk"
assert_file_contains "$builder" \
    "zte-usb-wifi-manager_0\\.1\\.0_rc1-r13_all\\.ipk"
...
case " $* " in
    ...
    *' package/zte-usb-wifi-manager/compile '*)
        ...
        printf 'apk-backend\n' \
            >bin/packages/fixture/zte-usb-wifi-manager-0.1.0_rc1-r13.apk
        printf 'ipk-backend\n' \
            >bin/packages/fixture/zte-usb-wifi-manager_0.1.0_rc1-r13_all.ipk
        ;;
...
printf apk-backend >"$work/incoming/packages-25.12.5/zte-usb-wifi-manager-0.1.0_rc1-r13.apk"
printf apk-luci >"$work/incoming/packages-25.12.5/luci-app-zte-usb-wifi-manager-0.1.0_rc1-r3.apk"
printf ipk-backend >"$work/incoming/packages-24.10.7/zte-usb-wifi-manager_0.1.0_rc1-r13_all.ipk"
...
assert_success node scripts/assemble-openwrt-packages.js \
    "$work/incoming" "$work/dist-r13-tag" "$source_sha" v0.1.0-rc1-r13
assert_success node -e '
const fs = require("fs");
const manifest = JSON.parse(fs.readFileSync(process.argv[1]));
if (manifest.project_ref !== "v0.1.0-rc1-r13" ||
    manifest.project_tag !== "v0.1.0-rc1-r13")
    process.exit(1);
' "$work/dist-r13-tag/build-manifest.json"
...
mv "$work/wrong-version-incoming/packages-25.12.5/zte-usb-wifi-manager-0.1.0_rc1-r13.apk" \
...
assert_file_contains "$workflow" "      - 'v0\\.1\\.0-rc1-r13'"
if grep -Fqx "      - 'v0.1.0-rc1'" "$workflow"; then
    fail 'r13 workflow must not publish the historical release tag'
else
    pass
fi
if grep -Fiq 'read-only developer preview' "$workflow"; then
    fail 'r13 release notes must not claim the package is read-only'
else
    pass
fi
assert_file_contains "$workflow" 'Backend r13'
assert_file_contains "$workflow" 'HAS_LOGIN:false'
if grep -Eq 'Backend r[89]([^0-9]|$)' "$workflow"; then
    fail 'r13 workflow must not retain stale r8 or r9 release notes'
else
    pass
fi
...
assert_file_contains docs/validation/github-packages-and-qemu.md \
    'gh release download v0\.1\.0-rc1-r13'
assert_file_contains README.md 'v0\.1\.0-rc1-r13'
